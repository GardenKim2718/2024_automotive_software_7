...
ScenarioRunner::ScenarioRunner(const std::string& node_name, const rclcpp::NodeOptions& options)
    ...
    s_vehicle_state_ = this->create_subscription<ad_msgs::msg::VehicleState> (
        "vehicle_state", 
        qos_profile, 
        std::bind(&ScenarioRunner::CallbackVehicleState, this, std::placeholders::_1)
        );
    ...
    time_prev_ = this->now().seconds(); // Get current time using second as the unit
...
void ScenarioRunner::Run() {
    auto current_time = this->now(); // Get current_time of the system and assign to the variable
    ...
    time_prev_ = current_time.seconds(); // Get current_time of the system and assign to the variable using second as the unit
    ...
    // Publish output
    // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - //
    p_detected_mission_->publish(ros2_bridge::UpdateMission(scc_objects, 
        merge_objects, 
        road_condition, 
        road_slope, 
        speed_limit));
    
    p_display_mission_->publish(ros2_bridge::UpdateMissionDisplay(
        display_scc_objects, 
        display_merge_objects, 
        display_road_slope, 
        display_road_condition, 
        display_speed_limit));
}
